...
import ReactGlobeExample from './ReactGlobeExample';
import ChatWindow from './ChatWindow';
import worldBankApi from '../utils/api';

/* =====================================================================
   🦄  RECOMMENDED REFACTOR SKETCH (comment-only blueprint)            🦄
   -------------------------------------------------------------------

   This block illustrates how the upcoming v2 architecture should look.
   Keep it purely as comments so we preserve the working v1 logic below.

   ```jsx
   import { useEffect, useRef } from 'react';
   import GlobeContainer from './globe/GlobeContainer';
   import SidebarSearch   from './sidebar/SidebarSearch';
   import AvatarSettings  from './sidebar/AvatarSettings';
   import ChatWindow      from './chat/ChatWindow';
   import HamburgerButton from './sidebar/HamburgerButton';
   import { useGlobalState } from '../contexts/GlobalStateProvider';

   export default function RGE2 () {
     const globeRef = useRef(null);
     const [ state, actions ] = useGlobalState();

     // Bridge OpenAI function-calls ⇒ imperative globe methods
     useEffect(() => {
       const unsub = actions.subscribeToAI((fn) => {
         switch (fn.name) {
           case 'rotateGlobe':
             globeRef.current?.rotateTo(fn.args.lat, fn.args.lng, fn.args.durationMs);
             break;
           case 'highlightCountry':
             globeRef.current?.highlightCountry(fn.args.iso3);
             break;
           case 'addDatasetToGlobe':
             actions.loadAndAddDataset(fn.args.datasetId);
             break;
           case 'setYear':
             actions.setYear(fn.args.year);
             break;
           case 'toggleSidebar':
             actions.toggleSidebar(fn.args.panel);
             break;
           default:
         }
       });
     ...
     return (
       <div className="h-screen w-screen overflow-hidden text-white bg-black">
         <HamburgerButton />
         <SidebarSearch />
         <AvatarSettings />
         <GlobeContainer ref={globeRef} datasets={state.globeDatasets} />
         <ChatWindow />
       </div>
     );
   }
   ...
   ──────────────────────────────────────────────────────────────
   Additional Mock-Code (trimmed for brevity) ↓↓↓
   ...
   export const chatFunctions = [
     {
       name: 'rotateGlobe',
       description: 'Rotate 3-D globe to lat/lng',
       parameters: {
         type: 'object',
         properties: {
           lat: { type: 'number' },
   ...
   ```jsx
   // src/components/globe/GlobeContainer.jsx (excerpt)
   import { forwardRef, useImperativeHandle } from 'react';
   import Globe from 'react-globe.gl';

   const GlobeContainer = forwardRef(({ datasets }, ref) => {
     const globeRef = useRef();

     useImperativeHandle(ref, () => ({
       rotateTo(lat, lng, dur = 1000) {
         globeRef.current.pointOfView({ lat, lng }, dur);
       },
       highlightCountry(iso3) {
         // implementation
       }
     }));

     return <Globe ref={globeRef} globeImageUrl="/textures/earth-day.jpg" />;
   });
   ...
   ```jsx
   // src/components/sidebar/SidebarSearch.jsx (outline)
   export default function SidebarSearch() {
     ...
     useEffect(() => {
       ...
       const id = setTimeout(async () => {
         const res = await worldBankApi.searchDatasets(query);
         setResults(res);
       }, 300);
     ...
     return (
       <aside className="sidebar-search">{/* … */}</aside>
     );
   }
   ```

   Full blueprints for AvatarSettings, ChatWindow, HamburgerButton and hooks
   (`useGlobeActions`, `useDebounce`) are in plan.txt section V2.

   Questions answered by this blueprint:
     • Where to consolidate sprawling state 👉 GlobalStateProvider (Zustand).
     • Where to mount new SidebarSearch/AvatarSettings 👉 left drawer/toggle.
     • How AI drives UI 👉 function-calling contract → globeRef actions.
   ===================================================================== */
...
export default function RGE2() {
  ...
  // ==============================
  // V2 RENDER SCAFFOLD (temporary)
  // ==============================
  return (
    <div className="flex w-screen h-screen bg-black text-gray-100">
      {/* Left Rail */}
      <aside className={`flex flex-col justify-between bg-gray-900/50 backdrop-blur-lg z-20 transition-all duration-300 ${isRailOpen ? 'w-64' : 'w-16'}`}>
        {/* Top Icons */}
        <div className="flex items-center justify-between p-2">
          <button onClick={handleHamburger} className="p-1 hover:bg-gray-800 rounded">
            <span className="text-xl">☰</span>
          </button>
          <button onClick={handleChatIcon} className="p-1 hover:bg-gray-800 rounded">
            <span className="text-xl">💬</span>
          </button>
        </div>
        {/* Rail Content */}
        {mode === 'home' ? (
          <div className="flex-1 flex flex-col justify-between p-2">
            <div className="space-y-2">
              {/* Show on Globe / Graph Buttons */}
              <button
                onClick={() => console.log('Show on Globe')}
                className="w-full py-2 bg-blue-600 hover:bg-blue-500 rounded"
              >
                Show on Globe
              </button>
              <button
                onClick={() => console.log('Show Graph')}
              ...
              <div>
                <button
                  ...
                  className="w-full py-2 bg-purple-600 hover:bg-purple-500 rounded"
                >
                  Search World Bank Data
                </button>
                {searchOpen && (
                  <div className="mt-2 space-y-2">
                    <input
                      ...
                      placeholder="Keyword..."
                      className="w-full px-2 py-1 bg-gray-800 rounded border border-gray-600"
                    />
                    <ul className="max-h-32 overflow-y-auto bg-gray-800 rounded border border-gray-700">
            ...
            <div className="flex items-center space-x-2 mt-4">
              <input
                ...
                placeholder="Ask AI..."
                className="flex-1 px-2 py-1 bg-gray-800 rounded border border-gray-600"
              />
              <button
          ...
          <div className="flex-1 p-2 overflow-y-auto space-y-2">
            ...
            {chatHistory.map(conv => (
              <div
                ...
                className={`p-2 rounded cursor-pointer ${conv.id === currentConvId ? 'bg-gray-700' : 'hover:bg-gray-700'}`}
              >
      ...
      <main className="flex-1 overflow-auto">
        {mode === 'chat' ? (
          <ChatWindow
            ...
            onSend={handleFullSend}
            onAvatarClick={toggleAvatar}
            expandedAvatarId={expandedAvatarId}
          />
        ) : (
